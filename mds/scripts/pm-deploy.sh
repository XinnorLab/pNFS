...
#
# Usage: pm-deploy.sh <mode: legacy|rr|fill|smart> [extra mds.conf lines, ';'-separated]
#   smart  -> also prints placement_readiness / connector rows from config show
...
case "$MODE" in legacy|rr|fill|smart) ;; *) echo "usage: $0 <legacy|rr|fill|smart> [extra] | --rollback"; exit 2;; esac
...
sed -i -e '/^placement_/d' -e '/^ds_weight\./d' -e '/^ds_capacity_domain\./d' -e '/^ds_connector_/d' -e '/^# lattice-placement/d' /etc/pnfs-mds/mds.conf
